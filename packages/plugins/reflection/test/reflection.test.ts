...
function fakeEditor(): Fake {
  ...
  const publish = () => host.document.set({ id: state.docId, name: "score", dirty: false, version: state.version, measureCount: 4, staffCount: 2, title: "", tempo: null });
...
describe("state", () => {
  ...
  it("drops its captured base when the document closes", async () => {
    ...
    await flush();
    f.host.document.set({ id: "doc-1", name: "score", dirty: false, version: 9, measureCount: 4, staffCount: 2, title: "", tempo: null });
    await f.press();
    expect(f.executed).toEqual(["inversion", "inversion"]);
  });
});
